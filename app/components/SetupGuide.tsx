...
export const SetupGuide = ({ onDismiss, onStepComplete, items }: SetupGuideProps) => {
  const defaultExpandedIndex = items.findIndex((item) => !item.complete);
            ...
            <s-stack direction="inline">
              ...
              <s-button
                ...
                onClick={() => {
                  setIsGuideOpen((prev) => {
                    if (!prev) {
                      const incompleteIndex = items.findIndex((item) => !item.complete);
                      setExpanded(incompleteIndex !== -1 ? items[incompleteIndex].id : (items[0]?.id || null));
                    }
                    return !prev;
                  });
                }}
...
const SetupItem = ({
  ...
  const completeItem = async (e: any) => {
      ...
      <s-box borderRadius="small" background={expanded ? 'subdued' : undefined} paddingBlockStart="small-400" paddingInline="small-300" paddingBlockEnd="small-400">
        <s-grid gridTemplateColumns='auto 1fr' alignItems='start' columnGap='small'>
          <s-grid-item>
            ...
            <s-clickable onClick={completeItem} interestFor={`complete-tooltip-${id}`}>

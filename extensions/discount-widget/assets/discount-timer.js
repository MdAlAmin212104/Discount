"use strict";(function(){document.addEventListener("DOMContentLoaded",function(){const w=document.querySelectorAll(".discountflow-widget-container");if(w.length===0)return;w.forEach(e=>{const d=e.getAttribute("data-shop"),i=e.getAttribute("data-product-id"),l=e.getAttribute("data-money-format")||"${{amount}}";if(!d||!i)return;let s=e.getAttribute("data-variant-id")||"";h(e,d,i,s,l);function a(r){let t="";r.detail&&r.detail.variant&&r.detail.variant.id?t=String(r.detail.variant.id):r.target&&r.target.name==="id"&&(t=r.target.value),t&&t!==s&&(s=t,e.style.display="none",e.innerHTML="",h(e,d,i,s,l))}document.addEventListener("variant:changed",a),document.addEventListener("variantChange",a),document.querySelectorAll('select[name="id"], input[name="id"]').forEach(r=>{r.addEventListener("change",a)})});function h(e,d,i,l,s){const a=e.getAttribute("data-design-mode")==="true";let r=`/apps/discount-showcase/products?shop=${d}&productId=${i}`;l&&(r+=`&variantId=${l}`),fetch(r).then(t=>t.json()).then(t=>{if(t.error||!t.stages||t.stages.length===0){a?f(e):e.style.display="none";return}x(e,t,s)}).catch(t=>{console.error("Error loading discount timer:",t),a?f(e):e.style.display="none"})}function b(e,d){let i=parseFloat(e).toFixed(2);return d.replace("{{amount}}",i).replace("{{amount_no_decimals}}",Math.round(i)).replace("{{amount_with_comma_separator}}",i.replace(".",",")).replace("${amount}",i)}function C(e,d,i){let l=parseFloat(e);return i==="PERCENTAGE"?l*(1-parseFloat(d)/100):i==="FIX_AMOUNT"?parseFloat(d):i==="FIXED_DISCOUNT"?Math.max(0,l-parseFloat(d)):l}function x(e,d,i){const l=d.stages||[],s=d.settings||{},a=d.discountType||"PERCENTAGE",r=d.products&&d.products[0],t=parseFloat(r?r.originalPrice:e.getAttribute("data-product-price")||"0");if(isNaN(t)||t<=0)return;const n={title:s.welcomeHeading||"Limited time offer",subtitle:s.countdownText||"Sale ends in:",bgColor:s.bgColor||"#8b5cf6",textColor:s.textColor||"#ffffff",cardBgColor:s.cardColor||"#faf9f7",accentColor:s.accentColor||"#7c3aed",mutedColor:s.mutedColor||"#6b7280",borderColor:s.borderColor||"#e2dfd9",salePriceColor:s.salePriceColor||"#E63946",originalPriceColor:s.originalPriceColor||"#6B7280",borderRadius:s.borderRadius||16,maxWidth:s.maxWidth||580,paddingTop:s.paddingTop||40,paddingBottom:s.paddingBottom||40};if(e.style.setProperty("--sds-bg-color",n.bgColor),e.style.setProperty("--sds-bg-gradient",`linear-gradient(135deg, ${n.bgColor}, ${n.accentColor})`),e.style.setProperty("--sds-text-color",n.textColor),e.style.setProperty("--sds-card-color",n.cardBgColor),e.style.setProperty("--sds-accent-color",n.accentColor),e.style.setProperty("--sds-muted-color",n.mutedColor),e.style.setProperty("--sds-border-color",n.borderColor),e.style.setProperty("--sds-sale-color",n.salePriceColor),e.style.setProperty("--sds-original-color",n.originalPriceColor),e.style.setProperty("--sds-border-radius",n.borderRadius+"px"),e.style.setProperty("--sds-max-width",n.maxWidth+"px"),e.style.setProperty("--sds-padding-top",n.paddingTop+"px"),e.style.setProperty("--sds-padding-bottom",n.paddingBottom+"px"),s.customCss){let o=document.getElementById("discountflow-custom-css");o||(o=document.createElement("style"),o.id="discountflow-custom-css",document.head.appendChild(o)),o.innerHTML=s.customCss}const c=new Date,u=l.find(o=>new Date(o.startDate)<=c&&new Date(o.endDate)>=c);if(!u){e.getAttribute("data-design-mode")==="true"?f(e):e.style.display="none";return}const v=u.stageNumber,y=`${n.subtitle}`;let p='<div class="discountflow-timeline-wrapper">';p+=`
      ...
      `;let g='<div class="discountflow-releases-list">';l.forEach(o=>{if(o.stageNumber>v){const D=C(t,o.discountValue,a),M=l.find(I=>I.stageNumber===o.stageNumber-1)?.label||`Drop ${o.stageNumber-1}`,N=o.shippingNoteLeft||`DROP ${o.stageNumber} - OPENS AFTER ${M.toUpperCase()}`,L=o.phaseTitle||o.label||`Drop ${o.stageNumber}`,A=b(D,i),F=o.shippingNoteRight||`Ships in ~${o.stageNumber*15} days`;g+=`
            <div class="discountflow-release-row">
              ...
              <div class="discountflow-release-right">
                ...
                <div class="discountflow-release-shipping">${F}</div>
              </div>
            </div>
          `}});const E=`DROP ${l.length+1} - PUBLIC RELEASE`,T="Open To The Public",$=b(t,i),m=s.publicShipping!==void 0?s.publicShipping:"Ships in ~5-7 days";let P="";m&&m.trim()!==""&&(P=`<div class="discountflow-release-shipping">${m}</div>`),g+=`
        <div class="discountflow-release-row public-release">
          <div class="discountflow-release-left">
            <div class="discountflow-release-eyebrow">${E}</div>
            <div class="discountflow-release-title">${T}</div>
          </div>
          <div class="discountflow-release-right">
